...
import os, time, sys, ConfigParser, shutil, datetime, traceback, unicodedata

cSep = os.path.join('a','b')[1] #/ on unix, \\ on windows...
...
def clean(l, encoding='NFC'):
    #decode and clear \r\n from a list of bytestrings
    return list(map(lambda x: unicodedata.normalize(encoding, x.decode('utf')).rstrip(), l))
